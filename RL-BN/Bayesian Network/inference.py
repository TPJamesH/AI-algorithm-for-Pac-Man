...
def constructBayesNet(gameState: hunters.GameState):
    ...
    #combination of maximum horizontal distance (X_RANGE - 1) and maximum vertical distance (Y_RANGE - 1)
    max_manhat_dist = X_RANGE + Y_RANGE - 2
    variables = [GHOST0,OBS0,PAC ,OBS1,GHOST1]
    edges = [(GHOST0,OBS0), #GHOST0 TO OBS0
             (PAC,OBS0), # PAC TO OBS0 
             (PAC,OBS1), # PAC TO OBS1
             (GHOST1,OBS1) # GHOST1 TO OBS1
             ]
    variableDomainsDict = {
        ...
        OBS0: list(range( max_manhat_dist + MAX_NOISE + 1)),
        OBS1:  list(range( max_manhat_dist + MAX_NOISE + 1))
    }
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        #Exclude query variables from elimination order
        eliminationOrder =[var for var in eliminationOrder if var not in queryVariables]
        
        #Interleave joins and eliminates
        for eliminatedVariable in eliminationOrder:
            #Join all factors,including the current eleminatedVariable
            currentFactorsList, joinedFactor = joinFactorsByVariable(currentFactorsList, eliminatedVariable)
            ...
            #Summing out for elimination
            sum_out = eliminate(joinedFactor,eliminatedVariable)

            #update the list of factors without the ones that is containing the eliminated variable
            currentFactorsList = [factor for factor in currentFactorsList if eliminatedVariable not in factor.unconditionedVariables()]
            currentFactorsList.append(sum_out)
        

        #Create full joint distribution
        fullJoint = joinFactors(currentFactorsList)

        
        #Return normalization directly
